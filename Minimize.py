...
autograd.extend.defvjp(autograd.numpy.asarray,
...
class Minimize(object):
    ...
    I have made the following changes in the class from the original Minimize class in 
    Legume:
        - Annealing of learning rate: the learning rate decays as 
        exp(-2.3 * iteration / Nepochs)
        - Return the intermidiate parameter history for debugging
    """
    ...
    def adam(self,
        ...
        for iteration in range(Nepochs):
            ...
            if iteration < Nepochs - 1:
                self.params = self.params - step_size * np.exp(
                    -2.3 * iteration / Nepochs) * grad_adam

            self.param_history = np.append(self.param_history,
                                           np.array(self.params))
        ...
        return (self.params, self.of_list, self.param_history,
                self.t_elapsed_array)
